...
const Home = ({ setBookId, bookId }) => {
   ...
   const wishList = async (bookId) => {
    ...
    await axios
      .request({
        ...
        method: "POST",
        url: `http://localhost:3000/api/v1/user/wishlist/${bookId}`,
      })
  ...
  const sortData = (e)=>{
    const getData = async () => {
      const data = await axios.get(
        `http://localhost:3000/api/v1/books?sort=${e.target.value}`
      );
      setData(data.data.books);
    };
    getData();
  ...
  useEffect(()=>{
    const sortCategory = async ()=>{
      ...
      const data = await axios.get(
        `http://localhost:3000/api/v1/books?genre=${categoryString}`
      );
      setData(data.data.books);
    }
    sortCategory()
